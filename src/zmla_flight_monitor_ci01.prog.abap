*&---------------------------------------------------------------------*
*&  Include           ZMLA_FLIGHT_MONITOR_CI01
*&---------------------------------------------------------------------*
...
*        if_is_technical = abap_true ),
        id_field_name  = 'MANDT'
        if_is_technical = abap_true ),
        id_field_name  = 'CARRID'
        if_is_hotspot  = abap_true
        id_tooltip     = |'Click here to see the Airline Master Record'| ),
        id_field_name  = 'SEATSMAX_B'
        if_is_visible  = abap_false ),
        id_field_name  = 'SEATSMAX_F'
        if_is_visible  = abap_false ),
        id_field_name  = 'PAYMENTSUM'
...
*        iv_name = 'MONSTER_NUMBER' it_ranges = go_selections->s_numbr[] ).
        iv_name = 'CARRID' it_ranges = go_selections->s_carrid[] ).
        range_table_collector->add_ranges_for_name(
*        iv_name = 'NAME' it_ranges = go_selections->s_name[] ).
        iv_name = 'CONNID' it_ranges = go_selections->s_connid[] ).
...
*    iv_table_name = 'ZMONSTER_HEADER' )->fullscreen( )->display( ).
    iv_table_name = 'SFLIGHT' )->fullscreen( )->display( ).
...
*          ls_carrid     LIKE LINE OF go_selections->s_carrid,
*          ls_connid     LIKE LINE OF go_selections->s_connid.
*
*    LOOP AT go_selections->s_carrid INTO ls_carrid.
*      ls_selections-field  = 'CARRID'.
*      ls_selections-sign   = ls_carrid-sign.
*      ls_selections-option = ls_carrid-option.
*      ls_selections-low    = ls_carrid-low.
*      ls_selections-high   = ls_carrid-high.
*      APPEND ls_selections TO lt_selections.
*    ENDLOOP.
*
*    LOOP AT go_selections->s_connid INTO ls_connid.
*      ls_selections-field  = 'CONNID'.
*      ls_selections-sign   = ls_connid-sign.
*      ls_selections-option = ls_connid-option.
*      ls_selections-low    = ls_connid-low.
...
*    ENDLOOP.
    ...
    SELECT * FROM sflight INTO TABLE @DATA(lt_data)
      UP TO @go_selections->p_count ROWS
      WHERE carrid IN @go_selections->s_carrid
      AND   connid IN @go_selections->s_connid.

    "The two table are identical, except the ALV one has the CELLTAB
    "addition, to make cells editable
    LOOP AT lt_data ASSIGNING FIELD-SYMBOL(<ls_data>).
      APPEND INITIAL LINE TO et_output_data ASSIGNING FIELD-SYMBOL(<ls_output_data>).
...
* Local Variables
  ...
  METHOD fill_editable_fields.
    ...
    mt_editable_fields = VALUE #( ( 'SEATSOCC' ) ( 'SEATSOCC_B' ) ( 'SEATSOCC_F' ) ).
...
* Local Variables
    ...
    CASE ed_user_command.
      WHEN 'ZEDIT'."A command to make the grid editable
        ...
        EXPORTING id_column_name = 'SEATSOCC'
        CHANGING  ct_fcat        = field_catalog_table ),
        EXPORTING id_column_name = 'SEATSOCC_B'
        CHANGING  ct_fcat        = field_catalog_table ),
        EXPORTING id_column_name = 'SEATSOCC_F'
        CHANGING  ct_fcat        = field_catalog_table ).
...
*     Test
      IF <alv_output_row>-seatsocc GT 200.
        <editable_cell_structure>-style = cl_gui_alv_grid=>mc_style_disabled.
      ELSE.
        <editable_cell_structure>-style = cl_gui_alv_grid=>mc_style_enabled.
      ENDIF.
